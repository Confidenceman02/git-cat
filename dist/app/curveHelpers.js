...
const removePrefix = (value) => (value.slice(1));
export const extractCubicCurves = (allCoordiantes) => {
    ...
    for (let i = 0; i < splitCooridnates.length; i++) {
        if (isCubicCurve(splitCooridnates[i])) {
            cubicCurvePositions.push([removePrefix(splitCooridnates[i]), splitCooridnates[i + 1], splitCooridnates[i + 2]]);
        }
    }
